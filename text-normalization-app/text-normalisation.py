...
# coding: utf-8
# I built a new XGboost model to use the context to label data.
# 
...
# **Analysis**
# 
# XGBoost with context performs so well that it outperforms XGboost without context by a great margin. And what's more, **XGboost with context nearly perfectly fits to the training dataset it has seen.** I believe, if built larger and fed more data, it might be able to challenge deep RNN.
...
def context_window_transform(data, pad_size):
    ...
    # All data with ndarray of zeros before and after
    data_with_pre = pre + data + pre
    neo_data = []
    print("flag: ", np.arange(len(data_with_pre) - pad_size * 2))
    # loop through data not including padding and create list of windows
